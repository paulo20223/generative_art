import random
from .colors import random_palette
...
for _ in range(len(PALETTES) + 4):
    PALETTES.append(random_palette(n=5))
...
def random_color(palette):
    hex_color = random.choice(palette['colors'])
    return hex_to_tuple(hex_color)


def random_colors(palette, n):
    hex_color = random.choices(palette['colors'], k=n)
    return [hex_to_tuple(h) for h in hex_color]
